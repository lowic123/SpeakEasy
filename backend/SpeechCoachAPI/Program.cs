...
app.MapGet("/generate-behavioural-question", async ([FromServices] ChatClient chat, QuestionStore store) => {
    ...
    store.LastGeneratedQuestion = result.Value.Content[0].Text;
    return Results.Ok(new { question = result.Value.Content[0].Text });
});

//generate tech question
app.MapGet("/generate-technical-question", async ([FromServices] ChatClient chat, QuestionStore store) => {
...
//technical
app.MapPost("/analyze-tech-interview", async (HttpRequest request, [FromServices] AudioClient audio, [FromServices] ChatClient chat, QuestionStore store) =>
{
    return await ProcessVoiceLogic(request, audio, chat, store, isInterview : true, isTechInterview: true);
});

//behaviour
app.MapPost("/analyze-interview", async (HttpRequest request, [FromServices] AudioClient audio, [FromServices] ChatClient chat, QuestionStore store) =>
{
    return await ProcessVoiceLogic(request, audio, chat,store, isInterview: true,isTechInterview: false);
});

//general speech
app.MapPost("/analyze", async (HttpRequest request, [FromServices] AudioClient audio, [FromServices] ChatClient chat, QuestionStore store) =>
{
    return await ProcessVoiceLogic(request, audio, chat, store, isInterview: false, isTechInterview: false);
});
    

app.Run();

async Task<IResult> ProcessVoiceLogic(HttpRequest request, AudioClient audio, ChatClient chat, QuestionStore store, bool isInterview, bool isTechInterview)
{
    if (!request.HasFormContentType)
        return Results.BadRequest("Invalid data");
    var form = await request.ReadFormAsync();
    var file = form.Files.FirstOrDefault();
    Console.WriteLine($"DEBUG: Received Question = '{store.LastGeneratedQuestion}'");
    ...
    AudioClient audioClient = new("whisper-1", apiKey);
    ...
    ClientResult<AudioTranscription> transcriptionResult = await audioClient.TranscribeAudioAsync(audioPath, transcriptionOptions);
    ...
    string systemPrompt = isInterview ? isTechInterview ?
                            $"This is was the question asked: {store.LastGeneratedQuestion}" +
    ...
    string userPrompt = (isInterview? $@"The question asked was {store.LastGeneratedQuestion}" : "") +
        $@"
    ...
    var feedbackObj = System.Text.Json.JsonSerializer.Deserialize<AiFeedback>(feedbackJson);
    ...
    {
        Message = $"analysis complete : {store.LastGeneratedQuestion}",
...
{
    var ffprobePath = @"C:\Users\Owner\AppData\Local\Microsoft\WinGet\Packages\Gyan.FFmpeg_Microsoft.Winget.Source_8wekyb3d8bbwe\ffmpeg-8.0.1-full_build\bin\ffprobe.exe";
...
{
    public string LastGeneratedQuestion { get; set; } = string.Empty;
}